...
module.exports = class FilterUtility {
    ...
    static getGroups(champion) {
        ...
        /** Skins on Sale */
        if (champion.storeItems.some((item) => item.sale !== null))
            groups.push('filter_sale')

        /** Eternals Series 1 not owned */
        if (champion.statStone) {
            const series1 = champion.statStone.sets.find(
                (e) => e.name === 'Series 1'
            )

            console.log(champion.statStone.sets)

            if (series1.stonesOwned !== series1.stonesAvailable)
                groups.push('filter_eternals_unowned')
        }
    ...
    filter(filters, search = false) {
        ...
        this.shuffleInstance.filter(function (element, _) {
            ...
            let hasGroups = activeFilters.every((filter) =>
                groups.includes(filter)
            )

            if (search) {
                let championTitle = $(element)
                    .find('img')
                    .data('title')
                    .toLowerCase()
